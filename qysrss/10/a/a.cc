#include <bits/stdc++.h>

using namespace std;

const double eps = 1e-6;
const double PI = acos(-1.0);

struct Point {
	double x, y;
	Point(double xx = 0, double yy = 0): x(xx), y(yy) {};
	Point operator - (const Point p1) {
		return Point(x - p1.x, y - p1.y);
	}

	double operator ^ (const Point p1) {
		return x * p1.x + y * p1.y;
	}	
};

inline int sign (double d) {
	if (d > eps) return 1;
	else if (d < -eps) return -1;
	else return 0;
}

double dis(Point a, Point b) {
	return sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y));
}

double cross(Point a, Point b) {
	return a.x * b.y - a.y * b.x;
}

double ptoline(Point tp, Point st, Point ed) {

	double t1 = dis(tp, st);
	double t2 = dis(tp, ed);
	double ans = min(t1, t2);
	if (sign((ed - st) ^ (tp - st)) >= 0 && sign((st - ed) ^ (tp - ed)) >= 0) {
		double h = fabs(cross(st - tp, ed - tp)) / dis(st, ed);
		ans = min(ans, h);
	}
	return ans;
}

double xx1, yy1, xx2, yy2;

double ptoRec(Point tp) {
	Point p1(xx1, yy1);
	Point p2(xx1, yy2);
	Point p3(xx2, yy1);
	Point p4(xx2, yy2);

	double ans = ptoline(tp, p1, p2);
	ans = min(ans, ptoline(tp, p2, p4));
	ans = min(ans, ptoline(tp, p4, p3));
	ans = min(ans, ptoline(tp, p3, p1));
	return ans;
}

double x, y, R;
double xx0, yy0;

Point get_Point(double A) {
	return Point(x + R * cos(A), y + R * sin(A));
}
double solve () {
	
	double l, r, mid, midmid;
	Point p0(xx0, yy0);
	Point p;
	l = 0; r = PI;

	while (r - l >= 1e-8) {
		mid = (l + r) / 2;
		midmid = (mid + r) / 2;
		Point p1 = get_Point(mid);
		Point p2 = get_Point(midmid);
		double t1 = ptoRec(p1) + dis(p0, p1);
		double t2 = ptoRec(p2) + dis(p0, p2);
		if (t1 > t2) l = mid;
		else r = midmid;
	}
	p = get_Point(l);
	double ans1 = dis(p0, p) + ptoRec(p);
//	out << ans1 << endl;

	l = PI; r = PI * 2;

	while (r - l >= 1e-8) {
		mid = (l + r) / 2;
		midmid = (mid + r) / 2;

		Point p1 = get_Point(mid);
		Point p2 = get_Point(midmid);
		double t1 = ptoRec(p1) + dis(p0, p1);
		double t2 = ptoRec(p2) + dis(p0, p2);
		if (t1 > t2) l = mid;
		else r = midmid;
	}

	p = get_Point(l);
	double ans2 = ptoRec(p) + dis(p0, p);
//	cout << ans2 << endl;
	return min(ans1, ans2);
}

int main() {
	
	while (scanf("%lf%lf", &xx0, &yy0) == 2) {
		if (xx0 == 0 && yy0 == 0) break;
		scanf("%lf%lf%lf", &x, &y, &R);
		scanf("%lf%lf%lf%lf", &xx1, &yy1, &xx2, &yy2);

		printf("%.2lf\n", solve());
	}

	return 0;
}
